...
Utils.prototype.close = function(){
	if(this.httpAgent){
		for(let key in this.httpAgent.sockets){
			let sockets = this.httpAgent.sockets[key];
			for(let j=0;j<sockets.length;j++){
				let socket = sockets[j];
				socket.destroy();
			}
		}
	...
	if(this.httpsAgent){
		for(let i=0;i<this.httpsAgent.sockets.length;i++){
			let sockets = this.httpsAgent.sockets[key];
			for(let j=0;j<sockets.length;j++){
				let socket = sockets[j];
				socket.destroy();
			}
